...
/**
 * Smart extraction: analyze uncommitted changes to infer current work
 * without relying on git commits (for developers who don't commit often)
 */
export async function extractFromUncommittedChanges(
    changes: { files: string[]; diff: string; changesSummary: Record<string, number> }
): Promise<Partial<ExtractedContext>> {
    const filesChanged = changes.files || [];
    const diffText = changes.diff || "";
    const changesSummary = changes.changesSummary || {};

    // Infer task from file changes
    const moduleNames = new Set<string>();
    for (const file of filesChanged) {
        const parts = file.split("/");
        if (parts.length > 1) {
            moduleNames.add(parts[0]); // First directory is usually module name
        }
    }

    let task = "";
    if (moduleNames.size > 0) {
        const modules = Array.from(moduleNames).slice(0, 3).join(", ");
        if (filesChanged.length > 1) {
            task = `Modifying ${modules} (${filesChanged.length} files)`;
        } else {
            task = `Working on ${filesChanged[0]}`;
        }
    } else {
        task = `Uncommitted changes (${filesChanged.length} files)`;
    }

    // Extract changes from diff
    const approaches: string[] = [];
    if (changesSummary && Object.keys(changesSummary).length > 0) {
        const changed = Object.entries(changesSummary)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 3)
            .map(([file, lineCount]) => `${file} (+${lineCount} lines)`)
            .join(", ");
        approaches.push(`Changed: ${changed}`);
    }

    // Detect patterns from diff (common refactoring patterns)
    ...
    if (removedLines > addedLines * 2) {
        approaches.push("Removing/cleaning up code");
    } else if (addedLines > removedLines * 2) {
        approaches.push("Adding new features/code");
    } else if (Math.abs(addedLines - removedLines) < 20) {
        approaches.push("Minor refactoring/fixes");
    }

    const currentState = `${filesChanged.length} files modified, ${addedLines} lines added, ${removedLines} removed`;
